...
namespace QHack {
	...
	byte Dungeon::existence_chance;
	DungeonComplex Dungeon::d;
	byte Dungeon::map[MAP_W][MAP_H];
	...
	void Dungeon::init_dungeon(void)
	{
	...
	void Dungeon::create_complete_dungeon(void)
	{
	...
	void Dungeon::dig_section(coord x, coord y)
	{
		...
		else
		{
			...
			do
			{
				d.s[d.dl][x][y].rx1 = x * SECT_W + SysDep::rand_byte(3) + 1;
				d.s[d.dl][x][y].ry1 = y * SECT_H + SysDep::rand_byte(3) + 1;
				d.s[d.dl][x][y].rx2 = (x + 1) * SECT_W - SysDep::rand_byte(3) - 2;
				d.s[d.dl][x][y].ry2 = (y + 1) * SECT_H - SysDep::rand_byte(3) - 2;
				...
				if (dir_possible(x, y, dir))
				{
					...
					case N:
						d.s[d.dl][x][y].dx[dir] = d.s[d.dl][x][y].rx1 +
							SysDep::rand_byte(room_width(x, y) - 1) + 1;
					...
					case E:
						d.s[d.dl][x][y].dy[dir] = d.s[d.dl][x][y].ry1 +
							SysDep::rand_byte(room_height(x, y) - 1) + 1;
	...
	int Dungeon::room_width(coord x, coord y)
	{
	...
	int Dungeon::room_height(coord x, coord y)
	{
	...
	byte Dungeon::rand_door(void)
	{
		byte roll = SysDep::rand_byte(100);
	...
	void Dungeon::connect_sections(coord sx1, coord sy1, coord sx2, coord sy2, byte dir)
	{
	...
	void Dungeon::dig_stairs(void)
	{
		...
		d.stxu[d.dl] = d.s[d.dl][sx][sy].rx1 +
			SysDep::rand_byte(room_width(sx, sy) - 1) + 1;
		d.styu[d.dl] = d.s[d.dl][sx][sy].ry1 +
			SysDep::rand_byte(room_height(sx, sy) - 1) + 1;
		...
		if (d.dl < MAX_DUNGEON_LEVEL - 1)
		{
			...
			do
			{
				x = d.s[d.dl][sx][sy].rx1 + SysDep::rand_byte(room_width(sx, sy) - 1) + 1;
				y = d.s[d.dl][sx][sy].ry1 + SysDep::rand_byte(room_height(sx, sy) - 1) + 1;
	...
	void Dungeon::get_random_section(coord *sx, coord *sy)
	{
		do
		{
			*sx = SysDep::rand_int(NSECT_W);
			*sy = SysDep::rand_int(NSECT_H);
	...
	void Dungeon::know(coord x, coord y)
	{
		if (is_known(x, y))
			return;
	...
	void Dungeon::print_tile(coord x, coord y)
	{
		map_cursor(x, y);
	...
	void Dungeon::print_tile_at_position(coord x, coord y)
	{
	...
	void Dungeon::know_section(coord sx, coord sy)
	{
		coord x, y;
	...
	void Dungeon::get_current_section_coordinates(coord px, coord py, coord *sx, coord *sy)
	{
		*sx = px / SECT_W;
		*sy = py / SECT_H;
	}
	...
	void Dungeon::get_current_section(coord px, coord py, coord *sx, coord *sy)
	{
	...
	char Dungeon::tile_at(coord x, coord y)
	{
		return map[x][y];
	}
	...
	void Dungeon::paint_map(void)
	{
		...
		{
			SysDep::cursor(0, 1 + y - d.psy * SECT_H);
	...
	void Dungeon::set_color_for_tile(char tile)
	{
		switch (tile)
		{
		case ROCK:
	...
	void Dungeon::change_door(coord x, coord y, byte door)
	{
		coord sx, sy;
		byte i;
	...
	void Dungeon::set_knowledge(coord x, coord y, byte known)
	{
		if (known)
